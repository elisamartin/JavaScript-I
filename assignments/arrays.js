...
for (let i = 0; i < inventory.length; i++) {
	const particularCar = inventory[i];
	if (particularCar.car_year < 2000) {
		oldCars.push(particularCar);
	}
}
...
for (let i = 0; i < inventory.length; i++) {
	const particularCar = inventory[i];
	if (particularCar.car_make === 'BMW' || particularCar.car_make === 'Audi') {
		BMWAndAudi.push(particularCar);
	}
}
